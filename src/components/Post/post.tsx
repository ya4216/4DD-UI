...
function Post() {
  ...
  const validationSchema = () => {
    return Yup.object().shape({
      title: Yup.string()
        .required('제목을 입력해주세요.')
        .test('len', '제목은 1 ~ 30 글자로 입력해주세요.', (val: any) => val && val.toString().length >= 1 && val.toString().length <= 30),
    });
  };
              ...
              {!state || isSwitch ? (
                <div>
                  ...
                  <div className="title_container">
                    <Field name="title" type="title" placeholder="제목을 30글자 이내로 입력해주세요." />
                  </div>
                  <div className="editor_container">
                    <EditorComponent getContent={getContent} contents={isSwitch && state.content} />
                  </div>
                  <div className="grid_button">
                    <Button type="submit" variant="contained" color="primary" style={{ width: '150px' }} disabled={!(pureContent && values.title) || values.title.length > 30}>
                      {isSwitch ? '수정하기' : '작성하기'}
                    </Button>
                  </div>
                </div>
                ...
                <div className="post_container">
                  ...
                  <div className="post_component">
                    <div>
                      <div className="post_title_box">
                        <div className="post_title">{state.title}</div>
                      </div>
                      <hr />
                      <div>
                  ...
                  {userInfo.name === state.userName && (
                    <div className="grid_button edit">
                      <Button className="btn_delete" variant="contained" onClick={handleClickOpen}>
                        삭제
                      </Button>
                      <Dialog open={open} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                        <DialogTitle id="alert-dialog-title">{state.title}</DialogTitle>
                        <DialogContent>
                          <DialogContentText id="alert-dialog-description">해당 게시물을 삭제하시겠습니까?</DialogContentText>
                        </DialogContent>
                        <DialogActions>
                          <Button
                            style={{
